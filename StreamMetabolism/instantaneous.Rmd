---
title: "Analysis of instantaneous stream metabolism parameters for East Fork Creek"
author: "Ming Chen"
...
pacman::p_load(tidyverse, skimr, zoo, lme4, readxl, broom, kableExtra)

```

# import the excel file that has the instantaneous measurement
```{r}
instantaneous <- read_excel("instantaneous_updated.xlsx")
# View(instantaneous)
```
...
# linear model of lnER(1/T) for each unique value of Date

```{r}

linmodel <- select(instantaneous, Date, inv_temp, log_ER_instant) %>%
  group_by(Date) %>%
  nest() 
...
GOF <- select(linmodel, Date, r.squared, p.value)
GOF$p.value <- format(GOF$p.value, digits = 3)
...
# linear model of GPP(PAR) for each unique value of Date

```{r}

linmodel <- select(instantaneous, Date, I, GPP.instant) %>%
  group_by(Date) %>%
  nest() 
GPP_model <- function(df) {
  lm(GPP.instant ~ I, data = df)
}

linmodel <- linmodel %>%
  mutate(model = map(data, GPP_model))
